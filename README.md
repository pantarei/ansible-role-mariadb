...
By default after installation with this role, MariaDB will still running under standalone by not in cluster. This is because the start/stop/restart must operate in a correct manner, and these actions can be handled by following sample Ansible playbooks.

To start (a.k.a. bootstrap) and new cluster, create a playbook as below (note that we seems the first node within current play as master node):
...
Restart nodes within a running cluster can simply done by [Rolling Update](http://docs.ansible.com/ansible/playbooks_delegation.html#rolling-update-batch-size):
